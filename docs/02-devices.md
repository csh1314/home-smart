...
## 七、中枢网关：Xiaomi 中枢网关（第二批购入）

### 音箱 Pro 不是中枢网关！

```
  ┌────────────────────────────────────────────────────────────┐
  │  ⚠️ 很多人以为音箱 Pro 就是中枢网关，这是误解！                │
  │                                                            │
  │  米家网关分三级：                                            │
  │                                                            │
  │  中枢网关 ── 大脑，本地存储自动化规则，断网也能执行            │
  │     │        代表：Xiaomi 中枢网关、BE6500 Pro 路由器         │
  │     │                                                      │
  │  从网关 ── 接受中枢调度，扩展蓝牙覆盖范围                     │
  │     │      代表：Xiaomi 智能音箱 Pro ← 你的音箱是这个级别     │
  │     │                                                      │
  │  盲网关 ── 只和云端通信，不参与本地自动化                      │
  │            代表：小爱音箱 mini                               │
  │                                                            │
  │  音箱 Pro 能做蓝牙 Mesh 2.0 网关连接设备，                   │
  │  但自动化场景的执行依赖云端 → 断网就废。                      │
  │  只有中枢网关才能实现本地自动化。                              │
  │                                                            │
  └────────────────────────────────────────────────────────────┘
...
│ 设备      │ 参考价    │ 说明                           │
├──────────┼──────────┼────────────────────────────────┤
│ Xiaomi   │ ～250 元  │ 唯一在售的独立中枢网关          │
│ 中枢网关  │ (大促     │ 本地自动化 + 极客版编程          │
│ ZSWG01CM │ 可到210)  │ 300台设备连接 + RJ45网口        │
└──────────┴──────────┴────────────────────────────────┘
...
```
  ...
  锐捷路由器（客厅网线口旁）
      │
      ├── 网线 ──→ Xiaomi 中枢网关（中枢，本地自动化）
      │              放客厅电视柜/弱电箱旁（靠网线口）
      │
      └── Wi-Fi ──→ Xiaomi智能音箱Pro（从网关+语音）
                     放客厅靠走廊侧（靠蓝牙覆盖+语音覆盖）

  三者各司其职：
  ┌────────────────────────────────────────────────────────┐
  │  中枢网关 ── 大脑：存储自动化规则，本地调度所有设备        │
  │  音箱 Pro ── 手脚：蓝牙 Mesh 2.0 覆盖 + 语音控制        │
  │  小爱 mini ── 补充：主卧语音控制（不参与中枢体系）        │
  └────────────────────────────────────────────────────────┘
...
```
  ...
  │  建议第二批和传感器一起买。理由：                              │
  │                                                            │
  │  第一批（开关+窗帘）→ 手动控制为主，音箱 Pro 网关够用          │
  │  第二批（传感器）→ 自动触发场景需要快速响应，中枢网关价值大    │
  │                                                            │
  │  大促时一起买：                                              │
  │  中枢网关(～210元) + 2个ES5(～120元) + 走廊传感器(～45元)    │
  │  = 约 375 元，解锁本地自动化 + 全屋传感器联动                │
  │                                                            │
  └────────────────────────────────────────────────────────────┘
```
...
│  夜灯：1 个 米家夜灯2（走廊插座）                          │
│  辅材：电笔/螺丝刀/胶布/空白面板                           │
│  第一批设备数：20 个 ｜ 预算：～1,895 元                    │
│                                                          │
│  第二批（住进去 1-2 周后，中枢网关+传感器一起买）            │
│  ───────────────────────────────────────                 │
│  中枢网关：1 个 Xiaomi 中枢网关（本地自动化）               │
